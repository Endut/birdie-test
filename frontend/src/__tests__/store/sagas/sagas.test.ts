...
describe('async actions', async () => {
  ...
  it('creates FETCH_VISITS_SUCCESS after fetchVisitsTask has been run', async () => {
    ...
    expect(fetchVisits).toHaveBeenCalledWith('123123', startDate, endDate);
    expect(dispatched).toEqual([fetchVisitsSuccess([mockVisitData])]);
  });
  ...
  it('creates FETCH_VISITS_ERROR after fetchVisitsTask has been run but with bad params', async () => {
    ...
    const dispatched = await recordSaga(fetchVisitsTask, fetchVisitsBegin('123124', startDate, endDate))
    expect(fetchVisits).toHaveBeenCalledWith('123124', startDate, endDate);
    expect(dispatched).toEqual([fetchVisitsError(new Error('no'))]);
  });
